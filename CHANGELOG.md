...
- [[#XXX](https://github.com/nf-core/differentialabundance/pull/XXX)] - Add `--normalisation_method` and a `LIMMA_NORMALISE` preprocessing step applying log2 plus limma `normalizeBetweenArrays()` (`scale`, `quantile`, `cyclicloess`, `vsn`) to the abundance matrix before differential analysis, plus a `mass_spec` profile for DIA mass spectrometry protein intensity matrices. ([@nikolaihecker](https://github.com/nikolaihecker)).
