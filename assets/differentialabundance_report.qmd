...
render_clustering_heatmaps <- function() {
    ...
    # Row labels are hidden on the heatmap but feed the cell mouseover, so
    # render them as "gene name (gene id)" where a name is available, giving
    # both identifiers on hover.
    hover_label <- function(ids) ids
    if (features_provided && !is.null(p$features_name_col) && p$features_name_col %in% colnames(features)) {
        id_vals    <- if (p$features_id_col %in% colnames(features)) features[[p$features_id_col]] else rownames(features)
        name_by_id <- set_names(as.character(features[[p$features_name_col]]), id_vals)
        hover_label <- function(ids) {
            nm <- name_by_id[ids]
            ifelse(is.na(nm) | nm == "" | nm == ids, ids, paste0(nm, " (", ids, ")"))
        }
    }
    ...
    walk(rev(names(assay_data)), \(assay_type) {
        ...
        print(htmltools::tagList(interactive_heatmap(
            ...
            cluster_rows      = FALSE,
            cluster_cols      = TRUE,
            row_labels        = hover_label(rownames(heat_mat)),
            show_row_labels   = FALSE,
            scale             = "none",
            grid_gap          = 0,
            cor_method        = p$exploratory_cor_method,
            cluster_method    = p$exploratory_clustering_method
        )))
        cat("\n\n")
    })
    cat(":::")
...
results <- map(differential_files, \(diff_file) {
    ...
    # read_differential returns sign-preserving linear fold changes
    # (sign(x) * 2^abs(x)); invert that to log2 for the volcano axis and
    # fold-change thresholds. Plain log2() would turn every down-regulated
    # (negative) value into NaN.
    fc <- diff[[p$differential_fc_column]]
    diff[[p$differential_fc_column]] <- ifelse(fc == 0, 0, sign(fc) * log2(abs(fc)))
...
# Name the outlier-point hover as "gene name (gene id)" where a name is available.
boxplot_labels <- NULL
if (features_provided && !is.null(p$features_name_col) && p$features_name_col %in% colnames(features)) {
    id_vals <- if (p$features_id_col %in% colnames(features)) features[[p$features_id_col]] else rownames(features)
    nm      <- as.character(features[[p$features_name_col]])
    boxplot_labels <- set_names(
        ifelse(is.na(nm) | nm == "" | nm == id_vals, id_vals, paste0(nm, " (", id_vals, ")")),
        id_vals
    )
}
interactive_boxplot(
