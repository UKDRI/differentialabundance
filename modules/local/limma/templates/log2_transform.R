...
# log2-transform only, with no between-sample normalisation. limma expects
# log-scale input, so this gives LIMMA_DIFFERENTIAL data it can model without
# imposing any between-sample correction. Column medians therefore stay
# unequal across samples - that is the point of this method.
...
write.table(
    mat_norm,
    file = paste(output_prefix, 'log2.tsv', sep = '.'),
    col.names = TRUE,
    row.names = FALSE,
    sep = '\t',
    quote = FALSE
